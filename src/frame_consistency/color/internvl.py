...
class InternVL:
    def __init__(self, quantization: bool, device: str="cuda", **kwargs) -> None:
        """ Class for the InternVL model.
        Args:   
        device (str): device to run the model
        kwargs: additional arguments
        """
        self.tokenizer = AutoTokenizer.from_pretrained("OpenGVLab/InternVL-Chat-V1-5-Int8", trust_remote_code=True)
    ...
    def run(self, frame, query):
        transform = self.build_transform(input_size=self.input_size)
        images = self.dynamic_preprocess(frame, image_size=self.input_size, use_thumbnail=True, max_num=self.max_num)
        pixel_values = [transform(image) for image in images]
        pixel_values = torch.stack(pixel_values).to(self.precision).to(device=self.device)
